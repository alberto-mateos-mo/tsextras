...
ts_season <- function(x, plot = TRUE, random = TRUE) {
  ...
  a <- (paste(start(x$trend.series)[1], 1, 1, sep = "/") %>% lubridate::ymd())+(start(x$trend.series)[2]-1)
  b <- (paste(end(x$trend.series)[1], 1, 1, sep = "/") %>% lubridate::ymd())+(end(x$trend.series)[2]-1)
